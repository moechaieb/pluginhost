...
namespace timeoffaudio {
    namespace scanner {
        ...
        public:
            Worker()
                : logger (juce::FileLogger::createDateStampedLogger ("time off audio", "PluginScanner", ".log", "")) {
        ...
        private:
            ...
            bool doScan (const juce::MemoryBlock& block) {
                ...
                logger->logMessage ("[timeoffaudio::scanner::Worker] doScan: formatName: " + formatName
                ...
                if (!juce::MessageManager::getInstance()->isThisTheMessageThread()
                    ...
                    logger->logMessage (
                        "[timeoffaudio::scanner::Worker] doScan failed. Requires unblocked message thread (format: "
                        + matchingFormat->getName() + ")");
                    return false;
                }

                if(!matchingFormat->fileMightContainThisPluginType(identifier)) {
                    logger->logMessage ("[timeoffaudio::scanner::Worker] doScan failed. fileMightContainThisPluginType returned false for identifier: " + identifier);
                    return false;
                }

                // if(matchingFormat->getName() == "VST3") {
                //     juce::VST3PluginFormat* vst3Format = static_cast<juce::VST3PluginFormat*>(matchingFormat);
                // }
                ...
                if(results.isEmpty()) {
                    logger->logMessage ("[timeoffaudio::scanner::Worker] [Warning] No plugin descriptions found for identifier: " + identifier);
                }
